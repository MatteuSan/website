...
export default function ToolPage({ children }) {
  return (
    <MarkdownLayout
      ...
      links={(
        <>
          <MSButton link={ToolInfo.links['docs']} type="small outlined">Docs</MSButton>
          <MSButton link={ToolInfo.links['figma']} type="small outlined">Figma</MSButton>
          <MSButton link={ToolInfo.links['src']} type="small outlined">Source</MSButton>
        </>
      )}
    >
      { children }
